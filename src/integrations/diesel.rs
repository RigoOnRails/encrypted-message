...
macro_rules! impl_from_and_to_sql {
    ($sql_type:ty, $backend:ty) => {
        impl<P: DeserializeOwned + Serialize + Debug, E: EncryptionType, C: Config> FromSql<$sql_type, $backend> for EncryptedMessage<P, E, C> {
        ...
        impl<P: DeserializeOwned + Serialize + Debug, E: EncryptionType, C: Config> ToSql<$sql_type, $backend> for EncryptedMessage<P, E, C> {
