...
macro_rules! impl_from_and_to_sql {
    ($sql_type:ty, $backend:ty) => {
        impl<P: DeserializeOwned + Serialize + Debug, E: EncryptionType> FromSql<$sql_type, $backend> for EncryptedMessage<P, E> {
        ...
        impl<P: DeserializeOwned + Serialize + Debug, E: EncryptionType> ToSql<$sql_type, $backend> for EncryptedMessage<P, E> {
